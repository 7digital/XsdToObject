...
namespace SevenDigital.Parsing.XsdToObject
{
	public class ClassWriter : IDisposable
	{
		...
		{
			_writer.WriteLine("\t\t\t{0} = element.Attributes().Where(a=>a.Name==\"{1}\").Select(a=>a.Value).FirstOrDefault();",
		...
		private void GenerateProperties(ClassInfo classInfo)
		{
			...
			{
				_writer.WriteLine("\t\tpublic string {0} {{ get; set; }}", NameUtils.ToCodeName(property, false));
			}
		}
